...
class SJSelectionSetRefleshSet(bpy.types.Operator):
    r"""Remove invalid object in set"""
    bl_idname = "sj_selection_set.refresh_set"
    bl_label = ""
    bl_description = "Refresh sets. (Remove invalid object in set)"
    bl_options = {'REGISTER', 'UNDO'}
...
class SJSelectionSetSaveList(bpy.types.Operator, bpy_extras.io_utils.ExportHelper):
    r"""Save List"""
    bl_idname = "sj_selection_set.save_list"
    bl_label = "Save"
    bl_description = "Save List"
    ...
    def invoke(self, context, event):
        ...
        f_name = os.path.splitext(os.path.basename(bpy.data.filepath))[0]
        self.filepath = os.path.join(f_path, f_name)
    ...
    def execute(self, context):
        ...
        list_data = {}
        for i, item in enumerate(context.scene.sj_sel_set_items, 0):
            list_data[item.set_name] = copy.copy(json.loads(
                context.scene.sj_sel_set_items[i].object_list,
                object_pairs_hook=collections.OrderedDict))

        save_file = open(self.filepath, 'w')
        json.dump(list_data, fp=save_file, indent=4)
        save_file.close()
        return {'FINISHED'}


class SJSelectionSetLoadList(bpy.types.Operator, bpy_extras.io_utils.ImportHelper):
    r"""Load List"""
    bl_idname = "sj_selection_set.load_list"
    bl_label = "Load"
    bl_description = "Load List"
    # bl_options = {'UNDO', 'PRESET'}

    directory: bpy.props.StringProperty()
    filename_ext = ".json"
    filter_glob: bpy.props.StringProperty(default="*.json", options={'HIDDEN'})
    # files: bpy.props.CollectionProperty(  # 複数ファイルの場合
    #         name="File Path",
    ...
    def execute(self, context):
        r""""""
        keywords = self.as_keywords(ignore=("filter_glob", "directory", "filepath"))
        # if not self.files:
        #     return {'CANCELLED'}
        ...
        load_file = open(self.filepath, 'r')
        list_data = json.load(
        ...
        current_names = [i.set_name for i in context.scene.sj_sel_set_items]
        for i in list_data:
            # TODO:同じことを複数回やってるので一つの関数にする
            new_item = context.scene.sj_sel_set_items.add()
            new_name = i
            cnt = 1
            while new_name in current_names:
                new_name = '{}.{:03d}'.format(new_name, cnt)
                cnt = cnt + 1
            new_item.set_name = new_name
            new_item.object_list = json.dumps(list_data[i])
...
class SJSelectionSetListPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        sub_row.operator("sj_selection_set.save_list", icon="FILE_TICK", text="")
        sub_row.operator("sj_selection_set.load_list", icon="FILEBROWSER", text="")
        sub_row.separator(factor=1)
        ...
        row.operator("sj_selection_set.refresh_set", icon="FILE_REFRESH")
        # CONSOLE RADIOBUT_OFF CHECKBOX_DEHLT FILE_REFRESH
...
classes = (
    ...
    SJSelectionSetRefleshSet,
    SJSelectionSetSelect,
    SJSelectionSetSaveList,
    SJSelectionSetLoadList,
